document.addEventListener('DOMContentLoaded', () => {
    ...
    // =========================================
    gsap.from(".left-panels", { duration: 1, x: -40, opacity: 0, ease: "power3.out", delay: 0.2 });
    gsap.from(".form-box", { duration: 1, x: 40, opacity: 0, ease: "power3.out", delay: 0.4 });


    // =========================================
    // 2. INFORMATION FADE SLIDER (BOTTOM LEFT)
    // =========================================
    const infoData = [
    ...
    // Initialize first text on load
    infoTitle.textContent = infoData[0].title;
    infoDesc.textContent = infoData[0].desc;

    // Auto-play text fade every 5 seconds (5000 milliseconds)
    setInterval(() => {
        // Fade out
        gsap.to(infoSlider, { opacity: 0, duration: 0.8, onComplete: () => {
            
            // Move to the next text item while it is invisible
